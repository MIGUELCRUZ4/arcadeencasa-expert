(() => {
  ...
  style.textContent = `
    #arcadeencasa-expert-widget { position:fixed; right:18px; bottom:10px; z-index:2147483000; font-family:'Courier New',Courier,monospace; pointer-events:none; }
    ...
    #arcadeencasa-expert-widget iframe { width:100%; height:100%; display:block; border:4px solid #17100d; border-radius:28px 28px 8px 28px; background:#fffdf1; box-shadow:9px 9px 0 #b2ebf2,15px 15px 0 rgba(74,4,4,.32); }
    #arcadeencasa-expert-widget .aec-comic-tail { position:absolute; right:-20px; bottom:22px; width:48px; height:44px; background:#fffdf1; border-right:4px solid #17100d; border-bottom:4px solid #17100d; transform:skewX(-28deg) rotate(13deg); z-index:2; }
    @keyframes aecMascotIdle { 0%,100% { transform:rotate(-1deg) translateY(0); } 50% { transform:rotate(1.5deg) translateY(-5px); } }
    ...
    @media (prefers-reduced-motion:reduce) { #arcadeencasa-expert-widget * { animation:none !important; transition:none !important; } }
  `;
